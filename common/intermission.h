...
struct OIntermissionDescriptor
{
	std::string link;
	std::vector<OIntermissionAction*> actions;

	~OIntermissionDescriptor()
	{
		for (std::vector<OIntermissionAction*>::iterator it; it != actions.end(); ++it)
			delete *it;
	}
};

typedef std::map<std::string, OIntermissionDescriptor*> OIntermissionDescriptorList;

extern OIntermissionDescriptorList IntermissionDescriptors;
