...
void ReplaceIntermission(const std::string& name, OIntermissionDescriptor* desc)
{
	OIntermissionDescriptor* pDesc = IntermissionDescriptors[name];
	if (pDesc != NULL)
		delete pDesc;
	IntermissionDescriptors[name] = desc;
}
} // namespace
...
namespace
{
void ParseIntermissionAction(OScanner& os, OIntermissionDescriptor* desc)
{
	...
	if (action)
		desc->actions.push_back(action);
}
} // namespace

void ParseIntermission(OScanner& os)
{
	os.mustScan();
	std::string name = os.getToken();
	OIntermissionDescriptor *desc = new OIntermissionDescriptor();

	ReplaceIntermission(name, desc);
	os.mustScan();
	os.assertTokenIs("{");
	...
	while (os.scan())
	{
		...
		ParseIntermissionAction(os, desc);
	}
}
