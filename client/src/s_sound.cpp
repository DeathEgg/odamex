...
void S_ChangeMusic(std::string musicname, int looping)
{
	...
	if (!(f = fopen (musicname.c_str(), "rb")))
	{
		...
		const size_t length = W_LumpLength(lumpnum);
		byte* data = static_cast<byte*>(W_CacheLumpNum(lumpnum, PU_CACHE));
		I_PlaySong(data, length, (looping != 0));
    }
    else
	{
		const size_t length = M_FileLength(f);
		byte* data = static_cast<byte*>(Malloc(length));
...
namespace
{
void SetTicker(int* tics, AmbientSound* ambient)
{
